...
BEGIN	
	...
	BEGIN TRY
		...
		-- Monat und Jahr - Zukunft oder laufender Monat
		IF (YEAR(GETDATE()) > @Jahr) OR (YEAR(GETDATE()) = @Jahr AND (MONTH(GETDATE()) > @Monat))
			THROW 50003, 'Monat und Jahr liegen in Vergangenheit, bitte korrigieren!', 1;

		-- INSERT INTO [dbo].[ProjektZeitBudget]
		INSERT INTO [dbo].[ProjektZeitBudget]
					([ProjektID], [Monat], [Jahr], [Budget], [Kommentar])
		 VALUES     (@ProjektID,  @Monat,  @Jahr,  @Budget,  @Kommentar);
